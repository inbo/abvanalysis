...
deze_analyse %>%
  ...
  inner_join(estimate, by = "analysis") %>%
  mutate(
...
deze_analyse %>%
  select(.data$analysis, .data$cycle) %>%
  inner_join(index, by = "analysis") -> deze_wijziging
stratum %>%
...
Op basis van driejaarlijkse gegevens zien we gemiddeld `r deze_lineair$tekst[deze_lineair$cycle]` `r deze_lineair$jaarlijks[deze_lineair$cycle]` per jaar of `r deze_lineair$looptijd[deze_lineair$cycle]` over de volledige looptijd van het meetnet.
Deze trend is `r deze_lineair$interpretatie[deze_lineair$cycle]`.

Op basis van jaarlijkse gegevens zien we gemiddeld `r deze_lineair$tekst[!deze_lineair$cycle]` `r deze_lineair$jaarlijks[!deze_lineair$cycle]` per jaar of `r deze_lineair$looptijd[!deze_lineair$cycle]` over de volledige looptijd van het meetnet.
Deze trend is `r deze_lineair$interpretatie[!deze_lineair$cycle]`.
...
(ref:{{id}}-sg) Stratumgewicht, raming aan het aantal hokken waarin de soort aanwezig is, aantal relevant hokken voor de analyse, aantal onderzochte hokken in het stratum, totaal aantal hokken van het stratum in Vlaanderen, aantal bezoeken aan een meetpunt en het gemiddeld aandeel relevante punten per hok voor `r soort_naam` (zie §\@ref(s:stratumgewicht)).
