...
Fig. \@ref(fig:inspanning) geeft de effectieve monitoringsinspanning weer.
...
Sommige hokken werden slechts in een of twee jaar onderzocht.
...
inspanning %>%
  ...
  arrange(start, end, jaren) %>%
  rowid_to_column("hok") %>%
  mutate(hok = factor(hok)) %>%
  inner_join(inspanning, by = "location") -> inspanning
ggplot(inspanning, aes(x = year, y = hok, group = hok)) +
  geom_line(alpha = 0.2) +
...
Hokken die slechts binnen één driejarige cyclus onderzocht zijn kunnen hierdoor voorlopig niet gebruiken.
Fig. \@ref(fig:potentieel) geeft een overzicht van het aantal hokken die gedurende minstens twee cycli onderzocht zijn.
