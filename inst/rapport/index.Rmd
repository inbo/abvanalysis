...
reportnr: "10.5281/zenodo.1494832"
link-citations: TRUE
...
aws <- aws.s3::get_bucket(
  "n2kmonitoring", file.path("abv", "manifest", fsep = "/"), max = Inf
)
map_chr(aws, "Key") %>%
  basename() %>%
...
tbl(conn, "location") %>%
  ...
  collect() %>%
  mutate(stratum = factor(stratum)) -> strata_description
strata_description %>%
...
git2rdata::read_vc(
  ...
  root = git2r::repository(file.path("~", "n2k", "abv"))
) %>%
