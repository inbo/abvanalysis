...
--[ insert year]
  if not is_empty(meta.flandersqmd.publicationdate) then
    z = z .. '<div>' .. '<div class="quarto-title-meta-heading">' ..
      ...
      pandoc.utils.stringify(meta.publicationyear) .. '</div>' .. '</div>'
  end
...
--[ insert citation]
  ...
  if is_empty(meta.flandersqmd.publicationdate) then
    z = z .. '<h1 class = "missing">!!! Missing flandersqmd.publicationdate !!!</h1>'
  else
    z = z .. ' (' .. meta.publicationyear .. ') '
  end
  if is_empty(meta.flandersqmd.title) then
    z = z .. '<h1 class = "missing">!!! Missing flandersqmd.title !!!</h1>'
  else
  ...
  z = z .. '<pre id = "bibtex" style ="display: none;">\n' .. '@TechReport{\n'
    ...
    if not is_empty(meta.flandersqmd.publicationdate) then
      z = z .. '  "issued":{"date-parts":[[' .. meta.publicationyear .. ']]},\n'
    end
    if not is_empty(meta.translation.name) then
