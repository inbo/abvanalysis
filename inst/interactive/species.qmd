...
plot_ly(estimate_year, x = ~year, y = ~estimate) |>
  add_fan(
    sd = ~sd, link = "log", text = ~display, hoverinfo = "text", name = "index"
  ) |>
  layout(
    xaxis = list(title = ""),
    yaxis = list(
    ...
    title = this_species, hovermode = "x unified"
  ) |>
  config(
...
voordien |>
  bind_rows(
    ...
    nadien |>
      ...
      mutate(
        alternative = .data$reference, estimate = 0, wijziging = "",
        interpretatie = "referentie"
      )
  ) -> jaarlijks
range(jaarlijks$reference) + c(-1, 1) -> bereik_x
c(
  jaarlijks$lower_confidence_limit, threshold,
  jaarlijks$upper_confidence_limit, -threshold
) |>
...
jaarlijks |>
  ...
  transmute(
    ...
    sd = .data$sd / 2 / qnorm(0.95),
    wijziging = sprintf(
      "%.0f t.o.v. %i\n%s\n%s", .data$alternative, .data$reference,
...
bscols(
  ...
  plot_ly(index_year, x = ~alternative, y = ~estimate, text = ~wijziging) |>
    add_fan(
      sd = ~sd, text = ~wijziging, name = "index", step = 0.3,
      showlegend = FALSE
    ) |>
    add_text(
...
bscols(
  ...
  plot_ly(index_cycle, x = ~alternative, y = ~estimate) |>
    add_fan(
      sd = ~sd, text = ~wijziging, hoverinfo = "text", step = 0.3,
      name = "index", showlegend = FALSE
    ) |>
    add_text(
      x = ~ref_x, y = ~ref_y, text = ~ref_text, textposition = ~textposition,
      color = I(inbo_hoofd), hoverinfo = "none", showlegend = FALSE
