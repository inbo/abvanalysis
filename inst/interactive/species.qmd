...
paste0(
  ...
  "(../methodologie/voorstelling.html#sec-onzekerheid) in stappen van 5%%. ",
  "Beweeg met de muis over de grafiek om de cijfers zichtbaar te maken."
) |>
...
paste0(
  ...
  "De band geeft het [90%% geloofwaardigheidsinterval]",
  "(../methodologie/voorstelling.html#sec-onzekerheid) in stappen van 30%%. ",
  "De [referentielijnen]",
  "(../methodologie/voorstelling.html#sec-trendklasse) ",
  "zorgen voor de opdeling van de wijzigingen in [klassen]",
  "(../methodologie/voorstelling.html#sec-trendklasse). ",
  "Beweeg met de muis over de grafiek om de indeling zichtbaar te maken."
) |>
...
## Analyse per jaar {#sec-jaarlijks}
  ...
  bind_rows(
    ...
    nadien |>
      ...
      mutate(
        ...
        estimate = 0,
        wijziging = "",
        interpretatie = "referentie"
      )
  ) -> jaarlijks
range(jaarlijks$reference) + c(-1, 1) -> bereik_x
c(
  jaarlijks$lower_confidence_limit,
  threshold,
  jaarlijks$upper_confidence_limit,
  -threshold
) |>
...
jaarlijks |>
  ...
  mutate(
    ...
    ref_text = c(
      "toestand in",
      "belangrijke afname t.o.v.",
      "belangrijke toename t.o.v."
    ) |>
      paste(head(.data$reference, 3)) |>
      c(rep("", n() - 3)),
    textposition = c("top left", "bottom left", "top left") |>
      c(rep("", n() - 3))
  ) |>
  SharedData$new(key = ~key) -> index_year
bscols(
  widths = c(10, 2),
  plot_ly(index_year, x = ~alternative, y = ~estimate, text = ~wijziging) |>
    add_fan(
      sd = ~sd,
      text = ~wijziging,
      name = "index",
    ...
    layout(
      ...
      yaxis = list(
        ...
        zerolinewidth = 1,
        tickvals = deze_breaks,
        ticktext = change_labels(deze_breaks)
      ),
      shapes = list(
        type = "rect",
        x0 = 0,
  ...
  filter_select(
    id = "ref_year",
    label = "Referentiejaar",
    sharedData = index_year,
    group = ~reference,
    multiple = FALSE
  )
) %>%
...
## Analyse per driejarige cyclus {#sec-driejaarlijks}
...
c(
  index_cycle$data(withFilter = FALSE)$lower_confidence_limit,
  threshold,
  index_cycle$data(withFilter = FALSE)$upper_confidence_limit,
  -threshold
) |>
  range(na.rm = TRUE) -> bereik
change_breaks(n = 5, extra = c(-threshold, threshold))(
