...
index_cyclus %>%
  ...
  mutate_at(c("median", "lcl", "ucl"), function(x) {
    -x
  }) %>%
...
if (nrow(index_cyclus)) {
  index_cyclus %>%
    ...
    ggplot(
      aes(x = naar, y = median, ymin = lcl, ymax = ucl, link_sd = sigma)
    ) +
...
model_validation %>%
  ...
  scale_y_continuous(paste("observed", "/", "expected ecdf", labels = percent)
```
